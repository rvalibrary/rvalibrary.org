...
class WpMediaFolder extends WpmfHelper
{
                    ...
                    if ($term->term_group == get_current_user_id()) {
                        if (in_array($term->term_id, $ts)) {
                            $attachment_terms[$term->term_id] = array(
                                ...
                                'files_count' => $count_file = $this->getCountFiles($term->term_id),
    ...
    public function preGetPosts1($query)
    {
        ...
        if ($pagenow == 'upload.php') {
            ...
            if ((int)$option_search == 0 || (empty($_GET['s']) && (int)$option_search == 1)) {
                ...
                if (isset($_COOKIE['lastAccessFolder'])) {
                    $selected = $_COOKIE['lastAccessFolder'];
                } else {
    ...
    public function adminHead()
    {
        ...
        $option_search = get_option('wpmf_option_searchall');
        $media_view = $this->getMediaView();
        if (isset($_GET['s']) && $_GET['s'] != '' && $option_search == 1 && $media_view == 'list') {
    ...
    public function getCountFiles($term_id)
    {
        global $wpdb;
        $sql = $wpdb->prepare(
            "SELECT COUNT(object_id) FROM " . $wpdb->prefix . "term_relationships WHERE term_taxonomy_id = %d ",
            array($term_id)
        );
                ...
                if ($wpmf_create_folder == 'user') {
                    if ($category->term_group == get_current_user_id()) {
                        if (isset($option_countfiles) && $option_countfiles == 1) {
                            $count_file = $this->getCountFiles($category->term_id);
                        }
                        $dirs[] = array(
                            'type' => 'dir',
                            'dir' => $dir,
