...
(function($) {
    wpmfFoldersModule =  {
        ...
        getFrame : function() {
            ...
            } else if (wpmfFoldersModule.page_type === 'upload-grid' || wp.media.frame === undefined) {
                if (wpmf.vars.base === 'toplevel_page_easingslider') { // work with Easing Slider plugin
                    return $('[id^="__wp-uploader-id-"]:visible div.media-frame').first();
                } else {
                    return $('div.media-frame').first();
                }
            } else {
                // Use wp.media.frame in case we have multiple frames (could be if multiple media managers have been opned)
                if (!$(wp.media.frame.$el).find('.media-frame-content .media-toolbar-secondary').length) {
                    return $(document);
                } else {
                    return wp.media.frame.$el;
                }
            }
        },
        ...
        initModule : function() {
            ...
            const init = function() {
                ...
                // input container folder id auto insert image gallery
                if (!$('.select_folder_id').length) {
                ...
                if (wpmfFoldersModule.page_type === 'upload-list') {
                    ...
                    wpmfFoldersModule.last_selected_folder = parseInt($('#wpmf-media-category').val());
        ...
        renderFolders: function(term_id) {
            ...
            for(let folder_id in wpmfFoldersModule.categories) {
                if (search === '') {
                    ...
                    let folder_name = wpmfFoldersModule.categories[folder_id].label;
            ...
            switch (wpmfFoldersModule.folder_ordering) {
                default:
                case 'name-ASC':
                    folders_ordered = folders_ordered.sort(function(a, b){return a.label.localeCompare(b.label);});
                    break;
                case 'name-DESC':
                    folders_ordered = folders_ordered.sort(function(a, b){return b.label.localeCompare(a.label);});
                    break;
                case 'id-ASC':
                    folders_ordered = folders_ordered.sort(function(a, b){return a.id - b.id});
                    break;
                case 'id-DESC':
                    folders_ordered = folders_ordered.sort(function(a, b){return b.id - a.id});
                    break;
                case 'custom':
                    folders_ordered = folders_ordered.sort(function(a, b){return a.order - b.order});
                    break;
            }
        ...
        openContextMenuFile: function () {
            ...
            $('.attachment:not(.wpmf-attachment)').bind('contextmenu', function (e) {
                wpmfFoldersModule.houtside();
                let x = e.clientX;     // Get the horizontal coordinate
                let y = e.clientY;
        ...
        initializeDragAndDropAttachments : function() {
            ...
            const $frame = wpmfFoldersModule.getFrame();
            let draggable_attachments = 'ul.attachments .attachment:not(.attachment.uploading):not(.wpmf-new):not(.wpmf-back):not(.ui-droppable):not(.ui-state-disabled)';
            let append_element;
                        ...
                        if ($('#wpmf_preview_image').length === 0) {
                            ...
                            if ((e.pageX + wpmfFoldersModule.hover_images[id_img].width) > $('body').width()) {
                                $("#wpmf_preview_image")
                                    ...
                                    .css("left", (e.pageX - wpmfFoldersModule.hover_images[id_img].width) - 50 + "px")
                                    .fadeIn("fast");
                            } else {
                                $("#wpmf_preview_image")
                                    .css("top", (e.pageY - 100 - $("#wpmf_preview_image").height()) + "px")
