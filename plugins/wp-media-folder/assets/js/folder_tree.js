    ...
    wpmfFoldersTreeModule = {
        ...
        loadTreeView: function loadTreeView() {
            ...
            wpmfFoldersTreeModule.getTreeElement().find('ul a[data-id]').droppable({
                ...
                drop: function drop(event, ui) {
                    ...
                    if ($(ui.draggable).hasClass('attachment') && !$(ui.draggable).hasClass('wpmf-attachment')) {
        ...
        getRendering: function getRendering() {
            ...
            var generateList = function generateList() {
                ...
                while (ij < wpmfFoldersTreeModule.categories.length) {
                    var className = '';
                    if (typeof lastStatusTree[wpmfFoldersTreeModule.categories[ij].id] !== "undefined" && lastStatusTree[wpmfFoldersTreeModule.categories[ij].id] === 'open' && typeof wpmfFoldersTreeModule.categories[ij].id !== "undefined") {
                        className += 'open ';
                    } else {
                        className += 'closed ';
                    }
        ...
        toggle: function toggle(folder_id) {
            ...
            if (wpmfFoldersTreeModule.getTreeElement().find('li[data-id="' + folder_id + '"]').hasClass('closed')) {
                ...
                lastStatusTree[folder_id] = 'open';
            } else {
                // Close the folder
                wpmfFoldersTreeModule.closeFolder(folder_id);
                // close all sub folder
                $('li[data-id="' + folder_id + '"]').find('li').addClass('closed');
                $('li[data-id="' + folder_id + '"]').find('li').each(function (i, v) {
                    lastStatusTree[$(v).data('id')] = 'close';
                });
                lastStatusTree[folder_id] = 'close';
            }
