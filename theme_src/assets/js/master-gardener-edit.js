...
function fetch(id, title, answer, categories){
  ...
  // catArr = catArr.join(',,,');
  jQuery.post('/wp-admin/admin-ajax.php', {'action': 'gravity_update', 'id': id, 'answer': answer, 'title': title, 'categories': catArr},
    function(data){
      if(data){
        ...
        console.log(data);
        setTimeout(function(){
          location.href = "https://rvalibrary.org/master-gardener/private-view-answered/";
        }, 1000);
      }
    })
}
