...
$age_group = get_query_var('slider_args');
$use_dropdown = get_query_var('use_dropdown');
$virtualCalendarId = 14747;
$age_group_type;

if(gettype($age_group) !== 'array') {
  echo gettype($age_group);
  $age_group_type = 'string';
  if($age_group === "children"){
    $age_group_cat_id = '4468,4469';
  } elseif($age_group === "teens") {
    $age_group_cat_id = '4471,4472';
  } elseif($age_group === "law") {
    $age_group_cat_id = 38052;
  } else {
    $age_group_cat_id = 4467;
  }
  echo $age_group_cat_id;
} else {
  $age_group_type = 'array';
}
...
if($age_group_type === 'string') {
  echo $age_group;
  $events_url = 'https://rvalibrary.libcal.com/1.1/events?cal_id=' . $virtualCalendarId . '&category=' . $age_group_cat_id . '&limit=5&days=120';
} else {
  echo $age_group;
  $categoryQueryString;
  $i = 1;
  foreach($age_group as $category) {
    if($i < count($age_group)) {
  ...
  echo $categoryQueryString;
  $events_url = 'https://rvalibrary.libcal.com/1.1/events?cal_id=' . $virtualCalendarId . '&category=' . $categoryQueryString . '&limit=5&days=120';
}
echo $event_url;
$events_args = array(
...
<?php endif; ?>
<?php if($age_group === "law"): ?>
<h1 class="press-header">Upcoming Events</h1>
<?php endif; ?>
  <div style="margin-top: 55px;" class="slider-container slider-container-top">
    ...
    <?php foreach ($events_array as $event): ?>
      ...
      <div class="carousel-img">
          ...
          <?php else: ?>
            <div class="img-container">
              <img src="https://images.unsplash.com/photo-1450704944629-6a65f6810cf2?ixlib=rb-1.2.1&auto=format&fit=crop&w=1047&q=80" class="img" style="max-height: 100%;">
            </div>
          <?php endif; ?>
  ...
  <?php else: ?>
    <h1 style="text-align:center;">No events at this location. Choose another library.</h1>
  <?php endif; ?>
  </div>
</div>
